 ...
 * 新建客户 客户已存在（存在于公海客户）
 */
public class ExistHighSeasDialog extends ICustomerToolDialog {
    ...
    public ExistHighSeasDialog(Context context) {
        super(context);
        setup();
    }
    ...
    private void setup() {
        ...
        mConfirmTextView = mContentView.findViewById(R.id.tv_positive);
        ...
        mContentView.findViewById(R.id.tv_negative).setOnClickListener(view -> dismiss());
    ...
    private void onSelectGroup() {
        ...
        LinearLayout.LayoutParams params;
        if (mCurrentGroupList.size() > SHOW_ENTRY) {  //最多显示6条
            params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, MAX_HEIGHT);
        } else {
            params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        }
    ...
    protected int bindContentView() {
        return R.layout.dialog_exist_highseas;
    }
}
