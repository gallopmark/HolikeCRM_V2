...
class BossMonthDataHelper extends MonthDataHelper {
    ...
    BossMonthDataHelper(BaseFragment<?, ?> fragment, Callback callback) {
        ...
        if (isAnimation) {
            if (mAction == null) {
                mAction = this::onQuery;
            }
            mFragmentView.postDelayed(mAction, 300);
        } else {
            onQuery();
        }
    }
    ...
    void onSuccess(MonthDataBossBean bean) {
        ...
        View firstLayout;
        if (TextUtils.equals(shop, "1") || TextUtils.equals(shop, "2")) {
            firstLayout = LayoutInflater.from(mActivity).inflate(R.layout.include_month_data_layout_first60dp, contentLayout, false);
        } else {
            firstLayout = LayoutInflater.from(mActivity).inflate(R.layout.include_month_data_layout_first80dp, contentLayout, false);
        }
        TextView tvFirst = firstLayout.findViewById(R.id.tv_first_name);
        contentLayout.addView(firstLayout, 1);  //动态添加layout 第一列数据展示布局
        final RecyclerView sideRecyclerView = mContentView.findViewById(R.id.rv_side);
        ...
        if (TextUtils.equals(shop, "1")) { //1小组数据 2员工数据 3员工收款数据
            ...
            tvFirst.setText(mActivity.getString(R.string.principal));
            List<MonthDataBossBean.ArrBean2> list = bean.getArr2();
            RecyclerView rvRole = mFragmentView.findViewById(R.id.rv_role);
            ...
            adapter.setOnItemClickListener((a, holder, view, position) -> {
                ...
                switchRole(position, list, sideRecyclerView, contentRecyclerView);
            });
            switchRole(0, list, sideRecyclerView, contentRecyclerView);
        } else { //员工收款数据
    ...
    private void switchRole(int position, List<MonthDataBossBean.ArrBean2> list, RecyclerView sideRecyclerView, RecyclerView contentRecyclerView) {
...
//        sideRecyclerView.setAdapter(new GeneralSideAdapter(mActivity, bean.getList()));
        ...
        } else if (TextUtils.equals(bean.type, "3")) {
            LinearLayout scrollableLayout = getScrollableLayout();
        ...
        } else if (TextUtils.equals(bean.type, "4")) {
            LinearLayout scrollableLayout = getScrollableLayout();
...
//            holder.setVisibility(R.id.tv_principal,View.INVISIBLE);
            ...
            holder.setText(R.id.tv_number_drawing, bean.scaleCount); //出图数
            ...
            holder.itemView.setOnClickListener(view -> mActivity.startFragment(new BossMonthDataFragment(), obtainBundle(bean.type, bean.cityCode, true)));
//            holder.itemView.setOnClickListener(view -> BossMonthDataActivity.open(mActivity, bean.type, bean.cityCode, bean.area));
    ...
    class ScrollableContentAdapter extends CommonAdapter<MonthDataBossBean.ArrBean> {
        ...
        public void onBindHolder(RecyclerHolder holder, MonthDataBossBean.ArrBean bean, int position) {
            ...
            holder.setOnClickListener(R.id.tv_received, view -> mActivity.startFragment(new BossMonthDataFragment(), obtainBundle(bean.type, bean.cityCode, false)));
            holder.setText(R.id.tv_received, bean.receivables); //已收款
            holder.setText(R.id.tv_contract_amount, bean.contractTotal); //成交总金额
        }
