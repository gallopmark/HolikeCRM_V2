...
/*首页帮助类*/
public class HomepageController implements HomeMenuFragment.OnMenuClickListener, View.OnClickListener {
    ...
    private HomepageControllerView mControllerView;
    ...
    public HomepageController(BaseActivity<?, ?> context, LinearLayout contentLayout, FragmentManager fragmentManager, HomepageControllerView controllerView) {
        this.mContext = context;
        MainDataSource.clear(mContext);
    ...
    public void onHttpOk(HomepageBean homepageBean, TextView tvRole) {
        resetStatus();
...
//        tvMsg.setText(HomePagePresenter.getMsgNum(bean.getNewData().getNewsCount()));
        ...
        if (mHomepageBean.getNewData() != null) {
            ...
            setSelectRole(mCurrentRoleIndex, tvRole);
    ...
    /*切换角色*/
    public void switchRole(final TextView tv) {
        ...
        popupWindow.showAtLocation(tv, Gravity.START | Gravity.TOP, mContext.getResources().getDimensionPixelSize(R.dimen.dp_6),
                mContext.getResources().getDimensionPixelSize(R.dimen.dp_35) + DensityUtil.getStatusHeight(mContext));
    }
    ...
    private void setSelectRole(int position, TextView tvRole) {
        ...
        if (tvRole.getVisibility() != View.VISIBLE) {
        ...
        if (mMonthDataView != null) {
            mMonthDataView.setOnClickListener(view -> {
                ...
                if (TextUtils.equals(userType, UserTypeValue.BOSS_V2)) {
                    //跳转老板本月数据页面
...
//                    if (i == 0) {
//                        FinanceMonthDataActivity.open(mContext);
//                    } else {
//                        InstallManagerMonthDataActivity.open(mContext);
//                    }
                }
            });
        }
    ...
    private void updatePage(HomepageBean.NewDataBean.RoleBean roleBean, boolean isEmptyMenu) {
        ...
        } else if (TextUtils.equals(userType, UserTypeValue.DESIGNER)) {    //设计师、设计经理
            updateOperateList(roleBean.getItemList());
...
//            isBoss = true;
    ...
    private void addMessageView(List<HomepageBean.MessageListBean> messageList, boolean isEmptyMenu) {
        ...
        tvMore.setOnClickListener(view -> mControllerView.onMessageItemClick(true, null));
        ...
        recyclerView.setAdapter(new CommonAdapter<HomepageBean.MessageListBean>(mContext, messageList) {
            ...
            public void onBindHolder(RecyclerHolder holder, HomepageBean.MessageListBean bean, int position) {
                ...
                holder.itemView.setOnClickListener(v -> mControllerView.onMessageItemClick(false, bean));
            }
        });
    ...
    /*private void updateBoss(HomepageBean.NewDataBean newDataBean) {
        setupDepositView(newDataBean);
        removeMonthDataView();
        mMonthDataView = mLayoutInflater.inflate(R.layout.include_homepage_month_data_boss, mContentLayout, false);
        setupAmount(mMonthDataView, newDataBean);
        TextView tvCustomers = obtainView(mMonthDataView, R.id.tv_customers);
        tvCustomers.setText(obtain(mContext.getString(R.string.number_of_new_customers), newDataBean.getNewsCount(), false));
        TextView tvSignatures = obtainView(mMonthDataView, R.id.tv_signatures);
        tvSignatures.setText(obtain(mContext.getString(R.string.number_of_signatures), newDataBean.getContractCount(), false));
        TextView tvMeasures = obtainView(mMonthDataView, R.id.tv_measures);
        tvMeasures.setText(obtain(mContext.getString(R.string.number_of_measuring), newDataBean.getScaleCount(), false));
        TextView tvStoreRate = obtainView(mMonthDataView, R.id.tv_storeRate);
        tvStoreRate.setText(obtain(mContext.getString(R.string.store_entry_turnover_rate), newDataBean.enterPercent, false));
        TextView tvGraphs = obtainView(mMonthDataView, R.id.tv_graphs);
        tvGraphs.setText(obtain(mContext.getString(R.string.number_of_output_graphs), newDataBean.getPicCount(), false));
        TextView tvRuleRate = obtainView(mMonthDataView, R.id.tv_ruleRate);
        tvRuleRate.setText(obtain(mContext.getString(R.string.scale_turnover_rate), newDataBean.scalePercent, false));
        ...
        tvDeposits.setVisibility(View.VISIBLE);
        tvDeposits.setText(obtain(mContext.getString(R.string.number_of_deposit_customers), newDataBean.ordersCustomer, false));
        ...
        tvSeasCount.setVisibility(View.VISIBLE);
        tvSeasCount.setText(obtain(mContext.getString(R.string.number_of_customers_into_seas), newDataBean.seaCustomer, false));
   ...
   /* private void updateDesigner(HomepageBean.NewDataBean newDataBean) {
        setupDepositView(newDataBean);
        removeMonthDataView();
        ...
        tvOrders.setText(obtain(mContext.getString(R.string.number_of_place_orders), newDataBean.getOrderCount(), false));
        TextView tvGraphs = obtainView(mMonthDataView, R.id.tv_graphs);
        tvGraphs.setText(obtain(mContext.getString(R.string.number_of_output_graphs), newDataBean.getPicCount(), false));
        TextView tvInstall = obtainView(mMonthDataView, R.id.tv_install);
        tvInstall.setText(obtain(mContext.getString(R.string.number_of_install), newDataBean.getInstalled(), false));
        TextView tvSignatures = obtainView(mMonthDataView, R.id.tv_signatures);
        tvSignatures.setText(obtain(mContext.getString(R.string.number_of_signatures), newDataBean.getContractCount(), false));
        TextView tvRuleRate = obtainView(mMonthDataView, R.id.tv_ruleRate);
        tvRuleRate.setText(obtain(mContext.getString(R.string.scale_turnover_rate), newDataBean.scalePercent, false));
    ...
    private void updateDesigner(HomepageBean.NewDataBean.RoleBean roleBean) {
        ...
        tvMeasures.setText(obtain(mContext.getString(R.string.number_of_measuring), roleBean.scaleCount, false));
        TextView tvOrders = obtainView(mMonthDataView, R.id.tv_orders);
        tvOrders.setText(obtain(mContext.getString(R.string.number_of_place_orders), roleBean.orderCount, false));
        TextView tvGraphs = obtainView(mMonthDataView, R.id.tv_graphs);
        tvGraphs.setText(obtain(mContext.getString(R.string.number_of_output_graphs), roleBean.picCount, false));
        TextView tvInstall = obtainView(mMonthDataView, R.id.tv_install);
...
//    private void setupAmount(View view, HomepageBean.NewDataBean newDataBean) {
//        TextView tvReceived = view.findViewById(R.id.tv_received);
//        tvReceived.setText(obtainAmount(mContext.getString(R.string.payments_received), newDataBean.receivables));
//        TextView tvAmount = view.findViewById(R.id.tv_amount);
//        tvAmount.setText(obtainAmount(mContext.getString(R.string.total_contract_amount), newDataBean.contractTotal));
//    }
...
//    private void setupDepositView(HomepageBean.NewDataBean newDataBean) {
//        clearDepositView();
...
//        TextView tvCollectDeposit = mViewCollectDeposit.findViewById(R.id.tv_collect_deposit);
//        tvCollectDeposit.setOnClickListener(this);
...
//    }
    ...
    private void setupDepositView(HomepageBean.NewDataBean.RoleBean roleBean) {
        clearDepositView();
        mViewCollectDeposit = mLayoutInflater.inflate(R.layout.include_homepage_collect_depositv2, mContentLayout, false);
        setOrderCountText(roleBean.orderCount);
        TextView tvCollectDeposit = mViewCollectDeposit.findViewById(R.id.tv_collect_deposit);
        tvCollectDeposit.setOnClickListener(this);
        mContentLayout.addView(mViewCollectDeposit, 0);
    }
    ...
    private void updateMarketing(HomepageBean.OrderDataBean orderDataBean) {
        ...
        pbLeft.setOnClickListener(v -> {
            ...
            mControllerView.onViewClicked(v, intent);
        });
    ...
    /*安装工或安装经理本月数据*/
  /*  private void updateInstall(HomepageBean.NewDataBean newDataBean) {
        removeMonthDataView();
        mMonthDataView = mLayoutInflater.inflate(R.layout.include_homepage_month_data_install_manager, mContentLayout, false);
        setMonthDateText(mMonthDataView, newDataBean.getTime());
        ...
        tvInstallNumber.setText(obtain(mContext.getString(R.string.number_of_install_customers), newDataBean.getInstalled(), true));
        tvRate.setText(obtain(mContext.getString(R.string.once_install_completion_rate), newDataBean.firstSuccess, true));
        tvInstallSquares.setText(obtain(mContext.getString(R.string.number_of_install_squares), newDataBean.getArea(), true));
        tvSatisfaction.setText(obtainSatisfaction(mContext.getString(R.string.customer_satisfaction_tips), newDataBean.Satisfied));
        mContentLayout.addView(mMonthDataView);
    }*/
    ...
    /*仓管本月数据*/
  /*  private void updateWarehouse(HomepageBean.NewDataBean newDataBean) {
        removeMonthDataView();
        ...
        tvPartsOfInput.setText(obtain(mContext.getString(R.string.number_of_scanned_input_parts), newDataBean.scan, true));
        mContentLayout.addView(mMonthDataView);
    }*/
    ...
    /*财务人员本月数据*/
  /*  private void updateFinance(HomepageBean.NewDataBean bean) {
        removeMonthDataView();
    ...
    /*导购、业务本月数据*/
    /*private void updateGuideBusiness(HomepageBean.NewDataBean newDataBean) {
        setupDepositView(newDataBean);
        removeMonthDataView();
        ...
        tvReservation.setText(obtain(mContext.getString(R.string.number_of_reservation_scale), newDataBean.prescaleCount, false));
        TextView tvStoreRate = obtainView(mMonthDataView, R.id.tv_storeRate);
        tvStoreRate.setText(obtain(mContext.getString(R.string.store_entry_turnover_rate), newDataBean.enterPercent, false));
        TextView tvOrders = obtainView(mMonthDataView, R.id.tv_orders);
        tvOrders.setText(obtain(mContext.getString(R.string.number_of_customer_orders), newDataBean.ordersCustomer, false));
        TextView tvRuleRate = obtainView(mMonthDataView, R.id.tv_ruleRate);
        tvRuleRate.setText(obtain(mContext.getString(R.string.scale_turnover_rate), newDataBean.scalePercent, false));
        mContentLayout.addView(mMonthDataView);
    }*/
    ...
    private void updateGuideBusiness(HomepageBean.NewDataBean.RoleBean roleBean) {
        ...
        tvStoreRate.setText(obtain(mContext.getString(R.string.store_entry_turnover_rate), roleBean.enterPercent, false));
        TextView tvOrders = obtainView(mMonthDataView, R.id.tv_orders);
        tvOrders.setText(obtain(mContext.getString(R.string.number_of_customer_orders), roleBean.ordersCustomer, false));
        TextView tvRuleRate = obtainView(mMonthDataView, R.id.tv_ruleRate);
        tvRuleRate.setText(obtain(mContext.getString(R.string.scale_turnover_rate), roleBean.scalePercent, false));
        mContentLayout.addView(mMonthDataView);
    }
    ...
    /*售后本月数据*/
   /* private void updateAfterSale(HomepageBean.NewDataBean bean) {
        removeMonthDataView();
    ...
    private SpannableString obtainSatisfaction(String origin, String content) {
        int start = origin.length();
        String source = origin + "\n" + (TextUtils.isEmpty(content) ? "-" : content);
        int end = source.length();
        source += " ★";
        ...
        ss.setSpan(new AbsoluteSizeSpan(mContext.getResources().getDimensionPixelSize(R.dimen.textSize_20), false), start, end, flags);
        ss.setSpan(new ForegroundColorSpan(ContextCompat.getColor(mContext, R.color.textColor4)), start, end, flags);
        ss.setSpan(new ForegroundColorSpan(ContextCompat.getColor(mContext, R.color.textColor13)), end, end2, flags);
        ss.setSpan(new AbsoluteSizeSpan(mContext.getResources().getDimensionPixelSize(R.dimen.textSize_24), false), end, end2, flags);
        return ss;
    }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.tv_collect_deposit) {
            ...
            Intent intent = new Intent(mContext, CollectDepositActivity.class);
            mControllerView.onViewClicked(view, intent);
        }
        if (view.getId() == R.id.llNewsLayout) {
    ...
    public void onMenuClick(String type, String name) {
        if (mHomepageBean != null) {
            mControllerView.onMenuClick(type, name);
        }
    }

    public interface HomepageControllerView {
