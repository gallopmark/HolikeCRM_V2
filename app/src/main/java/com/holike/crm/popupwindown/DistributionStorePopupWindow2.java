...
public class DistributionStorePopupWindow2 extends BasePopupWindow implements ShopListRequestHelper.ShopListCallback, View.OnClickListener {

    private Context mContext;
    private RecyclerView rv;
    private ShopListRequestHelper helper;
    private CustomerListBeanV2.CustomerBean mCustomerListBean;
    ...
    public DistributionStorePopupWindow2(Context context, CustomerListBeanV2.CustomerBean bean, StateCallback stateCallback) {
        super(context);
    ...
    private void init() {
        ...
        ((TextView) mContentView.findViewById(R.id.tv_username)).setText(mCustomerListBean.userName);
        //所在省市
        ((TextView) mContentView.findViewById(R.id.tv_in_provinces_cities)).setText(mCustomerListBean.site);
        //详细地址
        ((TextView) mContentView.findViewById(R.id.tv_detailed_address)).setText(mCustomerListBean.address);
        //客服备注
        ((TextView) mContentView.findViewById(R.id.tv_server_note)).setText(mCustomerListBean.digitalRemark);

        mContentView.findViewById(R.id.tv_no_note).setVisibility(TextUtils.isEmpty(mCustomerListBean.digitalRemark) ? View.VISIBLE : View.GONE);
        ...
        helper.requestStore(mCustomerListBean.dealerId);
            ...
            public void onBindHolder(RecyclerHolder holder, DistributionGuiderBean.GuideBean bean, int position) {
                ...
                holder.itemView.setOnClickListener(v -> {
                    ...
                    btnConfirm.setOnClickListener(v1 -> {
                        helper.updateStore(mCustomerListBean.houseId, mCustomerListBean.personalId, currentStoreBean.getShopId(), bean.getUserId());
                        dismiss();
                    });
                });
            }
