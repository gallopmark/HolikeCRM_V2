...
public class CalendarPickerView extends ListView {
    ...
    private int weekendTextColorResId;
...
//                res.getColor(R.color.calendar_text_active));
        headerTextColor = a.getColor(R.styleable.CalendarPickerView_tsquare_headerTextColor,
                ResourceCompat.getColor(context, R.color.calendar_text_active));
        ...
        weekendTextColorResId = a.getResourceId(R.styleable.CalendarPickerView_tsquare_weekendTextColor,
                R.color.calendar_weekend_text_selector);
        a.recycle();
...
//        }
        ...
        if (date != null) {
            ...
            if (selectedCells.size() == 0 || !selectedCells.get(0).equals(cell)) {
                cell.setRangeState(RangeState.FIRST);
            ...
            if (selectionMode == SelectionMode.RANGE && selectedCells.size() > 1) {
                ...
                Date firstDate = selectedCells.get(0).getDate();
                Date lastDate = selectedCells.get(1).getDate();
                Date start, end;
...
//        int endMonthIndex = cells.getIndexOfKey(monthKey(selectedCals.get(1)));
                int startMonthIndex, endMonthIndex;
                int firstIndex = cells.getIndexOfKey(monthKey(selectedCals.get(0)));
                int lastIndex = cells.getIndexOfKey(monthKey(selectedCals.get(1)));
                if (firstIndex > lastIndex) {
                    startMonthIndex = lastIndex;
                    endMonthIndex = firstIndex;
                } else {
                    startMonthIndex = firstIndex;
                    endMonthIndex = lastIndex;
                }
    ...
    private MonthCellWithMonthIndex getMonthCellWithIndexByDate(Date date) {
        ...
        if (monthCells == null) return null;
    ...
    private class MonthAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            monthView.init(months.get(position), cells.getValueAtIndex(position), displayOnly,
                    titleTypeface, dateTypeface, dayTextColorResId, weekendTextColorResId);
            return monthView;
        }
    }
